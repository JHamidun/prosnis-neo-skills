...
- уже готовая HTML-страница с анимацией, надо просто снять её в MP4/GIF → `video-export`;
- дисциплина движения и типовые провалы (заимствовано, MIT) → `references/borrowed-motion-discipline.md`.

**Граница с `video-export` — «сделай из HTML видео» ловят оба, а работа разная.**
Путает то, что в обоих случаях исходник — веб-страница. Но `video-export` — это **экспортёр
чужого результата**: анимация уже существует, Playwright дёргает у неё `window.__setTime(t)`,
снимает скриншот на кадр и склеивает ffmpeg'ом. Он ничего не сочиняет и работает только если у
сцены есть внешний хук времени (чистые CSS-keyframes он снять не может). Здесь ролика ещё нет:
скилл сам делает раскадровку, подбирает кадры из 153 рецептов, ставит звук и бит-синк и собирает
Remotion-проект, который рендерится СВОИМ детерминированным рендерером
(`npx remotion render`) — снимать Remotion покадровыми скриншотами браузера не надо и вредно.
Признак в формулировке: **«у меня есть html/анимация, выгрузи в mp4» → `video-export`;
«сделай ролик про продукт/из лендинга» → сюда.**
